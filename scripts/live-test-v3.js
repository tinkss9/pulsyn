// Live Integration Test v3 — Correct Docker credentials
...
async function testPostgreSQL() {
  ...
  try {
    const pool = new Pool({ host: 'localhost', port: 5432, database: 'testdb', user: 'test', password: 'test' });
    ...
    results.push({ connector: 'postgresql', status: 'FAIL', duration: Date.now() - start, error: err.message.substring(0, 100) });
  }
}
...
async function testMySQL() {
  const start = Date.now();
  try {
    const conn = await mysql.createConnection({ host: 'localhost', port: 3306, database: 'testdb', user: 'root', password: 'test' });
    ...
    results.push({ connector: 'mysql', status: 'FAIL', duration: Date.now() - start, error: err.message.substring(0, 100) });
  }
}
...
async function testMongoDB() {
  ...
  try {
    ...
    results.push({ connector: 'mongodb', status: 'FAIL', duration: Date.now() - start, error: err.message.substring(0, 100) });
  }
}
...
async function testRedis() {
  ...
  try {
    ...
    results.push({ connector: 'redis', status: 'FAIL', duration: Date.now() - start, error: err.message.substring(0, 100) });
  }
}
...
async function testMSSQL() {
  ...
  try {
    ...
    const pool = await mssql.connect({ server: 'localhost', port: 1433, database: 'master', user: 'sa', password: 'Test@12345', options: { encrypt: false, trustServerCertificate: true } });
    await pool.request().query('SELECT 1 as test');
    await pool.close();
    results.push({ connector: 'mssql', status: 'PASS', level: 'INTEGRATION_VALIDATED', duration: Date.now() - start, details: 'query ok' });
  } catch (err) {
    results.push({ connector: 'mssql', status: 'FAIL', duration: Date.now() - start, error: err.message.substring(0, 100) });
  }
}
...
async function testSupabase() {
  ...
  try {
    ...
    const triggers = await client.query("SELECT trigger_name FROM information_schema.triggers WHERE trigger_name LIKE '%pulsyn%'");
    client.release();
    await pool.end();
    results.push({ connector: 'supabase', status: 'PASS', level: 'INTEGRATION_VALIDATED', duration: Date.now() - start, details: tables.rows.length + ' tables, ' + triggers.rows.length + ' CDC triggers' });
  } catch (err) {
    results.push({ connector: 'supabase', status: 'FAIL', duration: Date.now() - start, error: err.message.substring(0, 100) });
  }
}
...
async function main() {
  ...
  console.log('=== RESULTS ===');
  console.log('');
  results.sort((a, b) => a.connector.localeCompare(b.connector)).forEach(r => {
    console.log(r.status + ' ' + r.connector.padEnd(15) + r.level.padEnd(25) + r.duration + 'ms  ' + (r.details || r.error || ''));
  });
