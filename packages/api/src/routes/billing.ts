// Pulsyn Billing Routes — async PostgreSQL backed
...
billingRoutes.get('/plans/:planId', (req: Request, res: Response) => {
  ...
  res.json({ data: { ...plan, priceFormatted: formatPrice(plan.price) } });
});

// ─── Subscriptions ─────────────────────────────────────────────

billingRoutes.get('/subscriptions/:orgId', async (req: Request, res: Response) => {
  const sub = await getSubscriptionByOrg(req.params.orgId);
  if (!sub) return res.status(404).json({ error: 'No active subscription found' });
  const plan = getPlan(sub.plan_id);
  res.json({
    data: {
      ...sub,
      plan: plan ? { name: plan.name, price: plan.price, priceFormatted: formatPrice(plan.price), features: plan.features } : null,
    },
  });
});
...
billingRoutes.post('/subscriptions', async (req: Request, res: Response) => {
  ...
  if (!organizationId || !planId || !email) {
    return res.status(400).json({ error: 'Missing required fields: organizationId, planId, email' });
  }
  const plan = getPlan(planId);
  if (!plan) return res.status(400).json({ error: `Invalid plan: ${planId}` });

  const existing = await getSubscriptionByOrg(organizationId);
  if (existing && existing.status === 'active') {
    return res.status(409).json({ error: 'Organization already has an active subscription' });
  }

  const subscription = await createSubscription({ organizationId, planId, email, name });
  res.status(201).json({
    data: { ...subscription, plan: { name: plan.name, price: plan.price, priceFormatted: formatPrice(plan.price) } },
  });
});

billingRoutes.put('/subscriptions/:id', async (req: Request, res: Response) => {
  const { planId, cancelAtPeriodEnd } = req.body;
  if (planId && !getPlan(planId)) {
    return res.status(400).json({ error: `Invalid plan: ${planId}` });
  }
  const sub = await updateSubscription(req.params.id, { planId, cancelAtPeriodEnd });
  if (!sub) return res.status(404).json({ error: 'Subscription not found' });
  const plan = getPlan(sub.plan_id);
  res.json({
    data: { ...sub, plan: plan ? { name: plan.name, price: plan.price, priceFormatted: formatPrice(plan.price) } : null },
  });
});

billingRoutes.post('/subscriptions/:id/cancel', async (req: Request, res: Response) => {
  const { immediate } = req.body;
  const sub = await cancelSubscription(req.params.id, immediate);
  if (!sub) return res.status(404).json({ error: 'Subscription not found' });
  res.json({
    data: sub,
...
billingRoutes.get('/usage/:orgId', async (req: Request, res: Response) => {
  const sub = await getSubscriptionByOrg(req.params.orgId);
  const plan = sub ? getPlan(sub.plan_id) : getPlan('community');
  if (!plan) return res.status(404).json({ error: 'Plan not found' });

  const summary = await getUsageSummary(req.params.orgId, {
...
billingRoutes.post('/usage', async (req: Request, res: Response) => {
  const { organizationId, metric, quantity } = req.body;
  if (!organizationId || !metric || quantity === undefined) {
    return res.status(400).json({ error: 'Missing required fields: organizationId, metric, quantity' });
  }
  ...
  await incrementUsage(organizationId, metric, quantity);
  res.json({ data: { organizationId, metric, quantity, recordedAt: new Date().toISOString() } });
});

billingRoutes.get('/usage/:orgId/overage', async (req: Request, res: Response) => {
  const sub = await getSubscriptionByOrg(req.params.orgId);
  const plan = sub ? getPlan(sub.plan_id) : getPlan('community');
  if (!plan) return res.status(404).json({ error: 'Plan not found' });
  ...
  const overage = {
    rowsReplicated: calculateOverage(summary.metrics.rowsReplicated.used, plan.metered.rowsReplicated.freePerDay, plan.metered.rowsReplicated.perUnit, plan.metered.rowsReplicated.unitSize),
    apiCalls: calculateOverage(summary.metrics.apiCalls.used, plan.metered.apiCalls.freePerDay, plan.metered.apiCalls.perUnit, plan.metered.apiCalls.unitSize),
    total: 0,
  };
  ...
  res.json({
    data: {
      ...
      plan: plan.name,
      overage: {
        rowsReplicated: { cents: overage.rowsReplicated, formatted: formatPrice(overage.rowsReplicated) },
        apiCalls: { cents: overage.apiCalls, formatted: formatPrice(overage.apiCalls) },
        total: { cents: overage.total, formatted: formatPrice(overage.total) },
      },
    },
  });
});
...
billingRoutes.post('/checkout', async (req: Request, res: Response) => {
  if (!isStripeConfigured()) {
    return res.status(503).json({ error: 'Stripe not configured. Set STRIPE_SECRET_KEY environment variable.' });
  }
...
billingRoutes.post('/portal', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ error: err instanceof Error ? err.message : 'Failed to create portal session' });
  }
});
