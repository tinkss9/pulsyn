// Asana Connector — Real API Integration
// Auth: Personal Access Token or OAuth2
// API: Asana API
// Asana REST API — project management, 150K+ paying customers
...
@registerSource('asana-real')
export class AsanaRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://app.asana.com/api/1.0';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Asana API key/token required');

    const resp = await this.apiGet('/ping') || await this.apiGet('/me') || await this.apiGet('/');
    if (!resp || !resp.ok) throw new Error('Asana connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['projects', 'tasks', 'users', 'teams', 'workspaces', 'portfolios'];
  }
