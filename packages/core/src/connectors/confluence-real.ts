// Confluence REST API v2 — Real API Integration
...
@registerSource('confluence-real')
export class ConfluenceRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{domain}.atlassian.net/wiki/api/v2';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Confluence REST API v2 API key/token required');
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Confluence REST API v2 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> { return ['pages', 'spaces', 'content', 'attachments', 'comments']; }
