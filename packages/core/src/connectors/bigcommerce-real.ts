// BigCommerce API v3 — Real API Integration
// Auth: Bearer
...
@registerSource('bigcommerce-real')
export class BigcommerceRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.bigcommerce.com/stores/{store_hash}/v3';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('BigCommerce API v3 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('BigCommerce API v3 connection failed');
    this.connected = true;
  }
