// LinkedIn API v2 — Real API Integration
// Auth: OAuth2 Bearer
...
@registerSource('linkedin-real')
export class LinkedinRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    ...
    if (!this.apiKey) throw new Error('LinkedIn API v2 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('LinkedIn API v2 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['people', 'organizations', 'posts', 'comments', 'shares'];
  }
