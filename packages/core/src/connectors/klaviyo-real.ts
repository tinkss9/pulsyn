// Klaviyo API v2 — Real API Integration
// Auth: Bearer
...
@registerSource('klaviyo-real')
export class KlaviyoRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://a.klaviyo.com/api';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Klaviyo API v2 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Klaviyo API v2 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['profiles', 'lists', 'segments', 'campaigns', 'flows', 'events'];
  }
