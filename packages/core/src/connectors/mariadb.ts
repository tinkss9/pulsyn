...
export class MariaDBConnector extends BaseConnector {
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    try {
      ...
      throw new Error(`MariaDB connection failed: ${(error as Error).message}`);
    }
  }
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      throw new Error(`MariaDB disconnect failed: ${(error as Error).message}`);
    }
  }
        ...
        for (const evt of events as any[]) {
          if (['Write_rows', 'Update_rows', 'Delete_rows'].includes(evt.Event_type)) {
            ...
            cb({
              ...
              before: op !== 'I' ? {} : null, after: op !== 'D' ? {} : null, ts: new Date(),
            });
          }
  ...
  async extractFull(table: string): Promise<UnifiedChangeEvent[]> {
    ...
    while (true) {
      ...
      for (const row of data) {
        events.push(createEvent('S', table, row, null, row[pk]?.toString() || null, { source: 'mariadb' }));
      }
  ...
  async extractIncremental(table: string, watermark: string | null): Promise<UnifiedChangeEvent[]> {
    ...
    for (const row of rows as any[]) {
      events.push(createEvent('I', table, row, null, row[wmCol]?.toString() || null, { source: 'mariadb' }));
    }
    return events;
  }
