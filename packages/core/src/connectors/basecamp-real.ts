// Basecamp API v4 — Real API Integration
// Auth: Bearer
...
@registerSource('basecamp-real')
export class BasecampRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://3.basecampapi.com/{account}';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Basecamp API v4 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Basecamp API v4 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['projects', 'todos', 'messages', 'people', 'schedules'];
  }
