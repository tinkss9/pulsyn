// Pinterest API v5 — Real API Integration
// Auth: Bearer
...
@registerSource('pinterest-ads-real')
export class PinterestAdsRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.pinterest.com/v5';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Pinterest API v5 API key/token required');
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Pinterest API v5 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> { return ['campaigns', 'adGroups', 'ads', 'audiences', 'catalogs']; }
