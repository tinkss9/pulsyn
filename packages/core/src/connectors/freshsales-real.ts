// Freshsales API — Real API Integration
// Auth: Bearer
...
@registerSource('freshsales-real')
export class FreshsalesRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{domain}.freshsales.io/api';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Freshsales API API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Freshsales API connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['contacts', 'accounts', 'deals', 'leads', 'appointments', 'tasks'];
  }
