// ActiveCampaign API v3 — Real API Integration
// Auth: Bearer
...
@registerSource('activecampaign-real')
export class ActivecampaignRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{domain}.api-us1.com/api/3';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('ActiveCampaign API v3 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('ActiveCampaign API v3 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['contacts', 'deals', 'campaigns', 'automations', 'tags'];
  }
