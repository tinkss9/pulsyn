// PayPal Connector — Real API Integration
// Auth: OAuth2 client credentials
// API: PayPal API
// PayPal REST API — 435M+ active accounts, global payment leader
...
@registerSource('paypal-real')
export class PaypalRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api-m.paypal.com/v1';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('PayPal API key/token required');

    const resp = await this.apiGet('/ping') || await this.apiGet('/me') || await this.apiGet('/');
    if (!resp || !resp.ok) throw new Error('PayPal connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['payments', 'invoices', 'subscriptions', 'catalogs', 'disputes', 'reports'];
  }
