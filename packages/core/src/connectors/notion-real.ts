// Notion Connector — Real API Integration
// Auth: Integration token (Bearer)
// API: Notion API
// Notion API — knowledge management + PM hybrid, 100M+ users
...
@registerSource('notion-real')
export class NotionRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.notion.com/v1';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Notion API key/token required');

    const resp = await this.apiGet('/ping') || await this.apiGet('/me') || await this.apiGet('/');
    if (!resp || !resp.ok) throw new Error('Notion connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['databases', 'pages', 'blocks', 'users', 'search'];
  }
