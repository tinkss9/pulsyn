// TikTok Marketing API v1.3 — Real API Integration
// Auth: Bearer
...
@registerSource('tiktok-ads-real')
export class TiktokAdsRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://business-api.tiktok.com/open_api/v1.3';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('TikTok Marketing API v1.3 API key/token required');
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('TikTok Marketing API v1.3 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> { return ['campaigns', 'adGroups', 'ads', 'audiences', 'pixels']; }
