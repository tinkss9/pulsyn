// Help Scout API v2 — Real API Integration
// Auth: Bearer
...
@registerSource('helpscout-real')
export class HelpscoutRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.helpscout.net/v2';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Help Scout API v2 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Help Scout API v2 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['conversations', 'customers', 'mailboxes', 'threads', 'tags'];
  }
