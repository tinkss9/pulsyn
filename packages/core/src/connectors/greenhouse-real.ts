// Greenhouse Harvest API v1 — Real API Integration
// Auth: Basic (API key)
...
@registerSource('greenhouse-real')
export class GreenhouseRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://harvest.greenhouse.io/v1';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Greenhouse Harvest API v1 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Greenhouse Harvest API v1 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['jobs', 'candidates', 'applications', 'interviews', 'offers'];
  }
