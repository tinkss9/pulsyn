// Microsoft Graph API — Real API Integration
// Auth: OAuth2 Bearer
...
@registerSource('microsoft-teams-real')
export class MicrosoftTeamsRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://graph.microsoft.com/v1.0';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Microsoft Graph API API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Microsoft Graph API connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['teams', 'channels', 'messages', 'members', 'apps'];
  }
