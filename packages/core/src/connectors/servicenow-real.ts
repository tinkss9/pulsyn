// ServiceNow Table API — Real API Integration
// Auth: Basic (API key)
...
@registerSource('servicenow-real')
export class ServicenowRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{instance}.service-now.com/api/now';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('ServiceNow Table API API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('ServiceNow Table API connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['incident', 'change_request', 'problem', 'task', 'sys_user'];
  }
