// Snapchat Ads API v1 — Real API Integration
// Auth: Bearer
...
@registerSource('snapchat-ads-real')
export class SnapchatAdsRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://adsapi.snapchat.com/v1';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Snapchat Ads API v1 API key/token required');
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Snapchat Ads API v1 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> { return ['campaigns', 'adSquads', 'ads', 'audiences', 'pixels']; }
