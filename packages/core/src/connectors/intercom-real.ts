// Intercom Connector — Real API Integration
// Auth: Bearer token or OAuth2
// API: Intercom API
// Intercom REST API — SaaS conversational support leader, 25K+ customers
...
@registerSource('intercom-real')
export class IntercomRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.intercom.io';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Intercom API key/token required');

    const resp = await this.apiGet('/ping') || await this.apiGet('/me') || await this.apiGet('/');
    if (!resp || !resp.ok) throw new Error('Intercom connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['contacts', 'conversations', 'companies', 'tags', 'segments', 'teams'];
  }
