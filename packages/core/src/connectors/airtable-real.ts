// Airtable API v0 — Real API Integration
// Auth: Bearer
...
@registerSource('airtable-real')
export class AirtableRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.airtable.com/v0';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Airtable API v0 API key/token required');
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Airtable API v0 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> { return ['bases', 'tables', 'records', 'views']; }
