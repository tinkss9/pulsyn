// Braintree GraphQL API — Real API Integration
// Auth: Basic (API key)
...
@registerSource('braintree-real')
export class BraintreeRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://payments.braintree-api.com/graphql';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Braintree GraphQL API API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Braintree GraphQL API connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['transactions', 'customers', 'subscriptions', 'plans', 'payment_methods'];
  }
