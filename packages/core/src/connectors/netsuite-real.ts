// NetSuite Connector — Real API Integration
// Auth: OAuth 1.0a or TBA (Token-Based Authentication)
// API: NetSuite API
// NetSuite REST API — mid-market ERP leader, 40K+ organizations
...
@registerSource('netsuite-real')
export class NetsuiteRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{account}.suitetalk.api.netsuite.com/services/rest/record/v1';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('NetSuite API key/token required');

    const resp = await this.apiGet('/ping') || await this.apiGet('/me') || await this.apiGet('/');
    if (!resp || !resp.ok) throw new Error('NetSuite connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['customer', 'salesOrder', 'invoice', 'item', 'vendor', 'employee', 'purchaseOrder', 'inventoryItem'];
  }
