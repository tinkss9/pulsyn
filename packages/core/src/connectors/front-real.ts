// Front API — Real API Integration
// Auth: Bearer
...
@registerSource('front-real')
export class FrontRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api2.frontapp.com';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Front API API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Front API connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['conversations', 'contacts', 'messages', 'tags', 'teammates'];
  }
