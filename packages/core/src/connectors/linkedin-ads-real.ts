// LinkedIn Marketing API v2 — Real API Integration
// Auth: OAuth2 Bearer
...
@registerSource('linkedin-ads-real')
export class LinkedinAdsRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.linkedin.com/v2';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('LinkedIn Marketing API v2 API key/token required');
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('LinkedIn Marketing API v2 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> { return ['campaigns', 'adAccounts', 'creatives', 'conversions', 'audiences']; }
