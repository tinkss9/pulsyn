// Monday.com Connector — Real API Integration
// Auth: API token (Bearer)
// API: Monday.com API
// Monday.com GraphQL API — fastest-growing PM tool, 225K+ customers
...
@registerSource('monday-real')
export class MondayRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.monday.com/v2';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Monday.com API key/token required');

    const resp = await this.apiGet('/ping') || await this.apiGet('/me') || await this.apiGet('/');
    if (!resp || !resp.ok) throw new Error('Monday.com connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['boards', 'items', 'updates', 'users', 'groups', 'columns'];
  }
