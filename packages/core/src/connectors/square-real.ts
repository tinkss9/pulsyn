// Square API v2 — Real API Integration
// Auth: Bearer
...
@registerSource('square-real')
export class SquareRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://connect.squareup.com/v2';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Square API v2 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Square API v2 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['payments', 'customers', 'orders', 'catalog', 'inventory', 'locations'];
  }
