// Linear API (GraphQL) — Real API Integration
// Auth: Bearer
...
@registerSource('linear-real')
export class LinearRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.linear.app/graphql';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Linear API (GraphQL) API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Linear API (GraphQL) connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['issues', 'projects', 'teams', 'cycles', 'labels', 'users'];
  }
