// SugarCRM REST API v11 — Real API Integration
// Auth: OAuth2 Bearer
...
@registerSource('sugarcrm-real')
export class SugarcrmRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{instance}.sugarondemand.com/rest/v11';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('SugarCRM REST API v11 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('SugarCRM REST API v11 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['Contacts', 'Accounts', 'Opportunities', 'Leads', 'Cases', 'Tasks'];
  }
