// Insightly API v3.1 — Real API Integration
// Auth: Basic (API key)
...
@registerSource('insightly-real')
export class InsightlyRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.insight.ly/v3.1';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Insightly API v3.1 API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Insightly API v3.1 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['contacts', 'organisations', 'opportunities', 'projects', 'tasks'];
  }
