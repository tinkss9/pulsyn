// Freshdesk Connector — Real API Integration
// Auth: API key (Basic auth)
// API: Freshdesk API
// Freshdesk REST API — fast-growing SMB support, 60K+ customers
...
@registerSource('freshdesk-real')
export class FreshdeskRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{domain}.freshdesk.com/api/v2';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Freshdesk API key/token required');

    const resp = await this.apiGet('/ping') || await this.apiGet('/me') || await this.apiGet('/');
    if (!resp || !resp.ok) throw new Error('Freshdesk connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['tickets', 'contacts', 'agents', 'companies', 'groups', 'satisfaction_ratings'];
  }
