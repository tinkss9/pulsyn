// Nationalize API — Nationality estimation from name (no auth)
...
export class NationalizeConnector extends BaseConnector {
  ...
  async getTableSchema(table: string): Promise<TableSchema> {
    return {
      ...
      columns: [
        ...
        { name: 'country', type: 'array', nullable: false },
      ],
      primaryKeys: ['name'],
    };
  }
  ...
  async extractFull(table: string): Promise<UnifiedChangeEvent[]> {
    ...
    for (const name of ['michael', 'emma', 'james', 'sarah', 'david']) {
      ...
      events.push(createEvent('nationalize', 'predictions', 'c', data, data.name));
    }
    return events;
  }
