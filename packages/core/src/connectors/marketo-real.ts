// Marketo Connector — Real API Integration
// Auth: OAuth2 client credentials
// API: Marketo API
// Marketo REST API — enterprise marketing automation
...
@registerSource('marketo-real')
export class MarketoRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{instance}.mktorest.com/rest';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Marketo API key/token required');

    const resp = await this.apiGet('/ping') || await this.apiGet('/me') || await this.apiGet('/');
    if (!resp || !resp.ok) throw new Error('Marketo connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['leads', 'campaigns', 'activities', 'programs', 'lists', 'smartlists'];
  }
