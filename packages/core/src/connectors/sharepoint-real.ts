// SharePoint REST API — Real API Integration
// Auth: OAuth2 Bearer
...
@registerSource('sharepoint-real')
export class SharepointRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{tenant}.sharepoint.com/_api';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('SharePoint REST API API key/token required');

    // Verify connection
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('SharePoint REST API connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['lists', 'items', 'files', 'folders', 'sites'];
  }
