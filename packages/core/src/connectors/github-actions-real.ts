// GitHub REST API v3 — Real API Integration
// Auth: Bearer (PAT)
...
@registerSource('github-actions-real')
export class GithubActionsRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://api.github.com';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('GitHub REST API v3 API key/token required');
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('GitHub REST API v3 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> { return ['workflows', 'runs', 'jobs', 'artifacts', 'secrets']; }
