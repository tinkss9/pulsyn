// Genderize API — Gender estimation from name (no auth)
...
export class GenderizeConnector extends BaseConnector {
  ...
  async testConnection(): Promise<boolean> {
    try {
      const res = await fetch(`${this.baseUrl}?name=james`);
      return res.ok;
    } catch { return false; }
  }
  ...
  async extractFull(table: string): Promise<UnifiedChangeEvent[]> {
    ...
    for (const name of ['james', 'emma', 'michael', 'sarah', 'david']) {
      const res = await fetch(`${this.baseUrl}?name=${name}`);
      const data = await res.json();
      events.push(createEvent('genderize', 'predictions', 'c', data, data.name));
    }
    return events;
  }
