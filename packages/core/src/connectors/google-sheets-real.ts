// Google Sheets API v4 — Real API Integration
// Auth: OAuth2 Bearer
...
@registerSource('google-sheets-real')
export class GoogleSheetsRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://sheets.googleapis.com/v4';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('Google Sheets API v4 API key/token required');
    const resp = await this.apiGet('/ping').catch(() => this.apiGet('/me').catch(() => this.apiGet('/')));
    if (!resp || !resp.ok) throw new Error('Google Sheets API v4 connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> { return ['spreadsheets', 'sheets', 'values']; }
