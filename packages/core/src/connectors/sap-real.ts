// SAP Connector — Real API Integration
// Auth: OAuth2 or Basic Auth
// API: SAP API
// SAP OData API — enterprise ERP leader, 440K+ customers
...
@registerSource('sap-real')
export class SapRealConnector extends BaseConnector {
  private baseUrl = '';
  private apiKey = '';
  ...
  async connect(config: DatabaseConfig): Promise<void> {
    this.config = config;
    this.baseUrl = config.host || 'https://{host}/sap/opu/odata/sap/API_BUSINESS_PARTNER';
    this.apiKey = config.token || config.password || config.apiKey || '';
    if (!this.apiKey) throw new Error('SAP API key/token required');

    const resp = await this.apiGet('/ping') || await this.apiGet('/me') || await this.apiGet('/');
    if (!resp || !resp.ok) throw new Error('SAP connection failed');
    this.connected = true;
  }
  ...
  async getTables(): Promise<string[]> {
    return ['A_BusinessPartner', 'A_SalesOrder', 'A_Customer', 'A_Vendor', 'A_Material', 'A_Invoice'];
  }
